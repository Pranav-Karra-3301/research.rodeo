...
type QuestionNodeType = Node<AnnotationNodeData, "question">;

function QuestionNodeInner({ data }: NodeProps<QuestionNodeType>) {
  const { annotation, onEdit, onDelete } = data;
  const [editing, setEditing] = useState(false);
  const [hovered, setHovered] = useState(false);
  const colors = ANNOTATION_COLORS["question"];
  ...
  return (
    ...
    >
      <Handle
        ...
        id="left"
        className="!bg-violet-400 !w-1.5 !h-1.5 !border-0 !opacity-0 group-hover:!opacity-100 transition-opacity"
      />
      ...
      >
        <div className="flex items-center gap-1.5 mb-1.5">
          <HelpCircle className="w-3.5 h-3.5 shrink-0" style={{ color: colors.border }} />
        ...
        {editing ? (
          <textarea
            autoFocus
            className="w-full text-[11px] resize-none bg-transparent outline-none border border-violet-300 rounded p-1"
...
export const QuestionNode = memo(QuestionNodeInner);
