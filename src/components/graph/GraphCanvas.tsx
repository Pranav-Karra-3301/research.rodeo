...
import { FrontierNode } from "./FrontierNode";
...
const nodeTypes = { paper: PaperNodeCard, frontier: FrontierNode } as any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const edgeTypes = { citation: CitationEdge, semantic: SemanticEdge, contradiction: ContradictionEdge } as any;
...
export function GraphCanvas() {
  ...
  const handleNodeClick: NodeMouseHandler = useCallback((e, node) => {
    ...
    setCtxMenu(null);
  }, [selectNode, toggleNodeSelection, setRightPanel]);
  ...
  return (
    <div className="w-full h-full relative">
      <ReactFlow
        ...
        onNodesChange={handleNC} onEdgesChange={handleEC}
        ...
        fitView fitViewOptions={{ padding: 0.25, maxZoom: 1.25 }}
        minZoom={0.1} maxZoom={2.5}
        nodesDraggable onlyRenderVisibleElements
      ...
      {ctxMenu?.type === "node" && ctxMenu.nodeId && (
        <GraphContextMenu
          type="node" position={ctxMenu.position}
          nodeId={ctxMenu.nodeId} nodeTitle={ctxMenu.nodeTitle ?? ""}
          onExpand={(mode) => handleExpandNode(ctxMenu.nodeId!, mode)}
          onViewDetails={() => { selectNode(ctxMenu.nodeId!); setRightPanel("reader"); }}
          onFindSimilar={() => selectNode(ctxMenu.nodeId!)}
          onCopyTitle={() => navigator.clipboard.writeText(ctxMenu.nodeTitle ?? "")}
          onOpenPdf={() => {
            const nd = useGraphStore.getState().getNode(ctxMenu.nodeId!);
            if (nd?.data.openAccessPdf) window.open(nd.data.openAccessPdf, "_blank");
          }}
