...
import { createNodeFromUrl, type ScrapeResult } from "@/lib/utils/url-source";
...
interface GraphState {
  ...
  /** Set when a node is materialized so the UI can fit view to it */
  ...
  addNodeFromUrl: (url: string, scraped?: ScrapeResult) => void;
...
export const useGraphStore = create<GraphState>()((set, get) => ({
  ...
  addNodeFromUrl: (url, scraped?) =>
    set((state) => {
      const node = createNodeFromUrl(url, scraped);
      const materialized = Array.from(state.nodes.values()).filter(
      ...
      nodes.set(node.id, { ...nodeWithPosition, scores });
      return {
        nodes,
        lastMaterializedNodeId: node.id,
      };
    }),
